...
def get_sha1_thumbprint(cert_path):
...
def authentication_handler(topic, payload):
    ...
    for filename in os.listdir(CERTIFICATE_FILE_PATH):
        ...
        if re.match(CERTIFICATE_PATTERN,filename) and os.path.isfile(os.path.join(CERTIFICATE_FILE_PATH,filename)):
            ...
            primaryThumbprint = get_sha1_thumbprint(os.path.join(CERTIFICATE_FILE_PATH,filename))
            secondaryThumbprint = get_sha1_thumbprint(os.path.join(CERTIFICATE_FILE_PATH,filename))
            break
